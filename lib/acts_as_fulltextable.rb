  ...
  module FulltextableInstanceMethods
    ...
    def update_fulltext_record
      if eval self.class.fulltext_options[:conditions]
        ...
        FulltextRow.where(["fulltextable_type = ? AND fulltextable_id = ?", self.class.to_s, self.id]).update_all(["value = ?, parent_id = ?", self.fulltext_value, self.parent_id_value]) if !(self.class.fulltext_options[:check_for_changes]) || (row.value != self.fulltext_value) || (self.parent_id_value != row.parent_id)
      else
